...
*/
	...
	$.fn.smint = function( options ) {
		...
		return $('.smint a').each( function(index) {
			...
			//Fill the menu
			var id = $(this).attr("id");
			optionLocs.push(Array($("."+id).position().top-menuHeight-1, $("."+id).height()+$("."+id).position().top, id));
			...
			var stickyMenu = function(direction){
				...
				if (scrollTop > stickyTop) { 
					$('.navigation-bar').css({ 'position': 'fixed', 'top':0 }).addClass('fxd');	
				} else {
					$('.navigation-bar').css({ 'position': 'absolute', 'top':stickyTop }).removeClass('fxd'); 
				}   
